...
export default class tarot extends base {
  ...
  async divine () {
    ...
    for (let k in cards) {
      k = Number(k)
      let cardreply = []
      if (formation.is_cut && k == num - 1) cardreply.push(`切牌「${represent[k]}」\n`)
      else cardreply.push(`第${k + 1}张牌「${represent[k]}」\n`)

      cardreply = _.concat(cardreply, await this.getTextImg(cards[k]))
      msg.push(cardreply)
    }
